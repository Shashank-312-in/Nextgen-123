...
import { getSmsGateways, setSmsGatewayAutoSend, type SmsGateway } from "../../api/logs";
...
export function AccountPage({ user, onLoggedOut }: Props) {
  ...
  const [smsGateway, setSmsGateway] = useState<SmsGateway | null>(null);
  ...
  async function reload() {
    ...
    try {
      ...
      if (res.user.role === "HOD") {
        try { const gateways = await getSmsGateways(); const g = gateways[0] || null; setSmsGateway(g); setSmsAutoSend(Boolean(g?.auto_send)); } catch (_e) { /* SMS controls are non-blocking on account load. */ }
      }
    } catch (_err) {
      // Clean load fallback
      ...
      {staffUser?.role === "HOD" && (
        <div className="detail-box">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
            <div><h3 style={{ margin: 0 }}>SMS Gateway</h3><p style={{ margin: "6px 0 0", color: "var(--muted)", fontSize: 13 }}>Controls absentee queue behavior for your HOD scope.</p></div>
            <label style={{ display: "flex", gap: 10, alignItems: "center", fontWeight: 800 }}>
              <input type="checkbox" checked={smsAutoSend} disabled={!smsGateway || !smsGateway.active} onChange={async (e) => {
                if (!smsGateway) return;
                try { const r = await setSmsGatewayAutoSend(smsGateway.id, e.target.checked); setSmsAutoSend(r.auto_send); setSmsGateway({ ...smsGateway, auto_send: r.auto_send }); setNotice(r.auto_send ? "SMS auto-send enabled." : "SMS auto-send disabled; approval is required."); }
                catch (err) { setError(err instanceof ApiClientError ? err.message : "Could not change SMS auto-send"); }
              }} /> Auto-send
            </label>
          </div>
          <div style={{ marginTop: 10, color: "var(--muted)", fontSize: 12 }}>When enabled, eligible absentee SMS rows are approved automatically. Default is OFF.</div>
        </div>
      )}
